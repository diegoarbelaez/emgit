...
$sentencia_inter = "select * from pagos where fk_id_contrato=$fk_id_contrato order by id_pago desc limit 1";
$resultado_inter = mysqli_query($conexion, $sentencia_inter);
if (mysqli_num_rows($resultado_inter) == 0) {
    //Significa que no hay pagos en la tabla pagos y debe buscar en la tabla contrato para ver el valor de la liquidacion
    ...
    $saldo = $fila_contrato["valor"] - $valor_bruto;
} else {
...
$sentencia = "INSERT into pagos(fecha_pago,valor_bruto,descuentos,valor_neto,saldo,fk_id_contrato) values ('$fecha_pago',$valor_bruto,$descuentos,$valor_neto,$saldo,$fk_id_contrato)";
$resultado = mysqli_query($conexion, $sentencia);
if ($resultado) {
    //echo "Agregado correctamente a la base de datos";
    echo "Sentencia ->" . $sentencia_inter;
} else {
