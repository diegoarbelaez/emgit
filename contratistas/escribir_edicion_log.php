...
//Debes validar, si el archivo adjunto no viene, entonces igual permitir la actualización del hecho
//Si el adjunto viene, actualizarlo
//Debe validarse si quiere eliminar el archivo

if ($_FILES['foto']['size'] == 0 || $_FILES['foto']['error'] == 4 || empty($_FILES['foto'])) {
    //Si no hay archivo adjunto en el formulario, entonces solo actualizar el hecho
    $sentencia = "UPDATE log set hecho = '$log', calificacion = 0 where id_log=$id_log";
    //echo $sentencia;
    ...
    if (isset($messages)) {
        ...
        // Aqui no puede ser un update, porque si es update no puede hacerlo sobre un log que no existe
        // En otras palabras, no va a actualizar un log que no existe
        // En otras palabras, cuando el contratista no ha cargado actividad, no hace el update
        $sentencia2 = "select * from soportes where fk_id_log=$id_log";
        $resul_log = $conexion->query($sentencia2);
        if($resul_log->num_rows>0){
            $sentencia2 = "update soportes set ruta='$target_file' where fk_id_log=$id_log";
        }
        else {
        ...
        $correo_supervisor = $fila4->correo_supervisor1;
...
<head>
    ...
    <title>El Contratista ' . $nombre_contratista . ' acaba de actualizar un Hecho sobre su contrato</title>
</head>
<body>
    <p>Hecho:</p>
    <br>
    ...
    <br>
    <p>
    Ya puedes revisar el hecho y volverlo a calificar si ya lo habías calificado
    <br>
    <br>
    ...
    Recuerda! MegaReporte hace las cosas más fáciles para ti!
        ...
        $mail->Subject = 'Hecho actualizado por ' . $nombre_contratista;
